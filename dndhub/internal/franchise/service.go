...
type SimulationResult struct {
	...
	AppliedActivities []string `json:"applied_activities"`
	Roll              int      `json:"roll"`
	Modifier          int      `json:"modifier"`
	Description       string   `json:"description"`
	// Detailed transparancy
	BaseRevenue       int64   `json:"base_revenue"`
	BaseUpkeep        int64   `json:"base_upkeep"`
...
func (s *Service) SimulateFranchise(ctx context.Context, userID, franchiseID uuid.UUID, activities []ActivityRequest) (SimulationResult, error) {
	...
	landUpkeep := int64(float64(fw.PropertyValueCents) * 0.10)
	...
	workerExpensesIncludingUniqueWorkers := workerExpenses
	for _, w := range fw.Workers {
		workerExpensesIncludingUniqueWorkers += int64(w.MonthlyCostCents)
	}

	baseRevenue := int64(landRevenue + workerExpenses)
	baseUpkeep := int64(landUpkeep + workerExpensesIncludingUniqueWorkers)
	...
	for _, act := range validActivities {
		...
		case ActivityMarketing:
			addedVal := int(float64(worker.Charisma) * float64(baseRevenue) * 0.01)
		...
		case ActivityRestructuring:
			addedVal := int(float64(worker.Discipline) * float64(baseUpkeep) * 0.01)
	...
	if accountingReductionPct > 1.0 {
		accountingReductionPct = 1.0
	}
	...
	if totalUpkeepBasis < 0 {
		totalUpkeepBasis = 0
	} // Costs shouldn't be negative (unless that's a grant?)
	...
	if len(appliedMessages) > 0 {
		// simple join
		joined := ""
		for i, msg := range appliedMessages {
			if i > 0 {
				joined += ", "
			}
			joined += msg
		}
		hist.Activities = joined
	}

	if err := s.repo.CreateHistory(ctx, hist); err != nil {
		// We log or fail? Let's fail for safety
		return SimulationResult{}, fmt.Errorf("failed to save history: %w", err)
	}
	...
	return SimulationResult{
		...
		BaseUpkeep:        int64(baseUpkeep), // baseUpkeep is int64 in previous lines
		RevenueMultiplier: selectedRange.Mult,
	}, nil
}

func (s *Service) GetFranchiseHistory(ctx context.Context, userID, franchiseID uuid.UUID) ([]FranchiseHistory, error) {
	// Verify access
	_, err := s.repo.GetFranchiseByID(ctx, userID, franchiseID)
	if err != nil {
		return nil, err
	}
	return s.repo.GetHistoryByFranchise(ctx, franchiseID)
}
